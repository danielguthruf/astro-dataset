...
ENV VIRTUAL_ENV /opt/astro-dataset-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/astro-dataset/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/astro-dataset/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/astro_dataset/ && touch src/astro_dataset/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/astro-dataset/
